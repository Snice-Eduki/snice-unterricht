...
FK_MIN = 8        # weniger Material -> keine eigene Seite (kein Thin Content)
FK_MAX_CARDS = 120  # Ladezeit: Rest über den Finder
...
def render_fachklasse_pages():
    ...
    for (f, n), v in kombis:
        v = sort_mats(v)
        gesamt = len(v)
        gezeigt = v[:FK_MAX_CARDS]
        lines = collections.Counter(m["line"] for m in v)
        parts = []
        def pl(k, one, many): return f"{k} {one if k == 1 else many}"
        if lines.get("lesespur"): parts.append(pl(lines["lesespur"], "Lesespurgeschichte", "Lesespurgeschichten"))
        if lines.get("pt"): parts.append(pl(lines["pt"], "Prüfungstraining", "Prüfungstrainings"))
        if lines.get("lueckentext"): parts.append(pl(lines["lueckentext"], "Lückentext mit Hörverstehen", "Lückentexte mit Hörverstehen"))
        what = ", ".join(parts) if parts else f"{gesamt} Materialien"
        # interne Verlinkung: Nachbarklassen im selben Fach + Fachseite + Klassenseite
        nachbarn = [f'<a href="{fk_slug(f, k)}.html">Klasse {k}</a>' for k in range(1, 14)
                    if (f, k) in vorhanden and k != n]
        links = (f'<p class="more"><a href="{fach_slug(f)}.html">Alle Materialien {html.escape(f)}</a> · '
                 f'<a href="klasse-{n}.html">Alle Fächer Klasse {n}</a>'
                 + (" · " + " · ".join(nachbarn) if nachbarn else "") + "</p>")
        rest = ""
        if gesamt > FK_MAX_CARDS:
            rest = (f'<p class="more">Hier sind die {FK_MAX_CARDS} neuesten von {nfmt(gesamt)} Materialien. '
                    f'<a href="materialien.html?q={html.escape(f)}">Alle im Material-Finder ansehen</a></p>')
        grid_page(fk_slug(f, n),
                  f"Arbeitsblätter {f} Klasse {n} – mit Lösungen, sofort einsetzbar",
                  f"Arbeitsblätter {f} Klasse {n}",
                  f"{what} für {f} in Klasse {n}: kopierfertig, differenziert und mit Musterlösungen – "
                  f"lehrplanorientiert für alle Bundesländer und Schularten.",
                  f"Fertige Arbeitsblätter für {f} in Klasse {n}: {what}. Jedes Material ist kopierfertig, "
                  "enthält Lösungen und lässt sich ohne Vorbereitung einsetzen.",
                  gezeigt, [("Alle Materialien", SITE + "/materialien.html"),
                            (f, SITE + f"/{fach_slug(f)}.html")],
                  with_fach=False, with_kl=False, links_html=links,
                  itemlist_name=f"Arbeitsblätter {f} Klasse {n}", body_extra=rest)
    return kombis
...
<p><a class="btn" href="/index.html">Zur Startseite</a> · <a href="/lesespurgeschichten.html">Lesespurgeschichten</a> · <a href="{SHOP}" rel="noopener">Snice-Shop auf eduki</a></p>""")
    ...
    print(f"OK: {len(posts)} Artikel, {nmat} DE-Materialien im Finder, {len(faecher)} Fach-Seiten, {len(klassen)} Klassen-Seiten, "
          ...
          f"{len(EXTRA_URLS)} Landingpages gesamt ({len(fk)} Fach×Klasse), {n_matpages} Material-Detailseiten.")
